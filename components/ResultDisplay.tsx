...
export const ResultDisplay: React.FC<ResultDisplayProps> = ({ swappedImage, loading, error, onReset }) => {
  ...
  return (
    <div className="w-full bg-gray-800 border border-gray-700 rounded-xl p-6 mt-8">
      <h3 className="text-2xl font-bold text-center mb-6 text-white">Sonuç</h3>
      {loading && (
        <div className="flex flex-col items-center justify-center h-80">
          <Spinner />
          <p className="text-indigo-400 mt-4 animate-pulse">Yapay zeka sihrini kullanıyor... bu biraz zaman alabilir.</p>
        </div>
      )}
      {error && (
